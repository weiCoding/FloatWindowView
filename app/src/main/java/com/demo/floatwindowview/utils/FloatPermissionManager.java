...
public class FloatPermissionManager {
    ...
    public boolean applyFloatWindow(Context context, UphoneCallback callback) {
        if (checkPermission(context)) {
            return true;
        } else {
            applyPermission(context,callback);
            return false;
        }
    }
    ...
    private void applyPermission(Context context,UphoneCallback callback) {
        if (Build.VERSION.SDK_INT < 23) {
            if (RomUtils.checkIsMiuiRom()) {
                System.out.println("jdsip applyPermission checkIsMiuiRom");
                miuiROMPermissionApply(context,callback);
            } else if (RomUtils.checkIsMeizuRom()) {
                System.out.println("jdsip applyPermission checkIsMeizuRom");
                meizuROMPermissionApply(context,callback);
            } else if (RomUtils.checkIsHuaweiRom()) {
                System.out.println("jdsip applyPermission checkIsHuaweiRom");
                huaweiROMPermissionApply(context,callback);
            }
        }

        System.out.println("jdsip applyPermission commonROMPermissionApply");
        commonROMPermissionApply(context,callback);
    }

    private void huaweiROMPermissionApply(final Context context,final UphoneCallback callback) {
    ...
    private void meizuROMPermissionApply(final Context context,final UphoneCallback callback) {
        showConfirmDialog(context, new OnConfirmResult() {
            @Override
            public void confirmResult(boolean confirm) {
                if (confirm) {
    ...
    private void miuiROMPermissionApply(final Context context,final UphoneCallback callback) {
        showConfirmDialog(context,"您的手机没有授予显示悬浮窗权限和后台弹出界面权限，请开启后再试",new OnConfirmResult() {
            @Override
            public void confirmResult(boolean confirm) {
                if (confirm) {
     ...
     * 通用 rom 权限申请
     */
    private void commonROMPermissionApply(final Context context,final UphoneCallback callback) {

        if (Build.VERSION.SDK_INT >= 23) {
            String message= "您的手机没有授予悬浮窗权限，请开启后再试";
            if(RomUtils.checkIsMiuiRom()){
                message= "您的手机没有授予显示悬浮窗权限和后台弹出界面权限，请开启后再试";
            }
            showConfirmDialog(context,message, new OnConfirmResult() {
                @Override
                public void confirmResult(boolean confirm) {
                    if (confirm) {
                        try {
